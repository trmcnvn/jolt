...
#[derive(Subcommand)]
enum DaemonCommand {
    /// Install, enable, and start the service (captures JOLT_* env).
    Install,
...
/// Production edge (Cloudflare Worker + Durable Objects on the trmcnvn.dev zone).
/// `JOLT_EDGE_URL` overrides (local dev / self-hosting).
const DEFAULT_EDGE_URL: &str = "https://edge.jolt.trmcnvn.dev";

/// Production WorkOS AuthKit client id — public knowledge (it appears in every
/// authorize URL), so baking it in is safe. Overridden by `JOLT_WORKOS_CLIENT_ID`;
/// set it to the empty string — or set a dev bearer via `JOLT_EDGE_TOKEN` — to
/// force dev-mode auth instead.
const DEFAULT_WORKOS_CLIENT_ID: &str = "client_01KZ8TAWMB4TZHF5RSTJPGR84J";

fn edge_url_from_env() -> String {
    std::env::var("JOLT_EDGE_URL")
        .ok()
...
/// otherwise a `JOLT_EDGE_TOKEN` dev bearer keeps dev mode (smoke tests,
/// local wrangler); otherwise the baked production client id — so a bare
/// `jolt headless` signs in against production with zero configuration.
fn workos_client_id_from_env(edge_token: &Option<String>) -> Option<String> {
    match std::env::var("JOLT_WORKOS_CLIENT_ID") {
...
/// Translate the Jolt-facing diagnostics knob to the inherited gpui fork's
/// internal name before any threads can read the process environment.
fn configure_dependency_env() {
    if let Some(value) = std::env::var_os("JOLT_GPU_STATS") {
        // SAFETY: this is the first operation in `main`, before logging,
        // runtimes, the UI, or any application-owned threads are initialized.
        unsafe { std::env::set_var(concat!("COM", "ET_GPU_STATS"), value) };
    }
}
...
/// `JOLT_HARNESS` (kebab-case id) picks the default harness for chats without a
/// config row — `mock` powers the e2e smoke; default `claude-code`.
fn harness_from_env() -> jolt_engine::HarnessId {
    match std::env::var("JOLT_HARNESS").as_deref().map(str::trim) {
...
/// `{data_dir}/logs/jolt-{mode}.log`, previous launch preserved as `.old`.
...
fn open_log_file_in(dir: &std::path::Path, mode: &str) -> Option<std::fs::File> {
    ...
    let path = dir.join(format!("jolt-{mode}.log"));
    #[cfg(unix)]
    {
        ...
        if rc != 0 {
            ...
            return std::fs::File::create(
                dir.join(format!("jolt-{mode}.{}.log", std::process::id())),
            )
            .ok();
        }
        ...
        drop(existing);
        if preexisting {
            let _ = std::fs::rename(&path, dir.join(format!("jolt-{mode}.log.old")));
        }
...
mod log_file_tests {
    ...
    fn second_launch_never_rotates_a_live_processes_log() {
        ...
        assert!(dir.join("jolt-headed.log").is_file());
        ...
        let pid_path = dir.join(format!("jolt-headed.{}.log", std::process::id()));
        assert!(pid_path.is_file(), "expected pid-suffixed overflow log");
        assert!(
            !dir.join("jolt-headed.log.old").exists(),
        ...
        assert!(
            dir.join("jolt-headed.log.old").is_file(),
            "rotation resumes"
        );
        drop(third);
    }
...
/// Delete `jolt-{mode}.{pid}.log` overflow files older than a week — they
