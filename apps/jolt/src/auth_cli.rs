//! `jolt login` / `jolt logout` / `jolt status` — the standalone auth surface.
//!
//! Sign-in used to live only inside `jolt headless`, coupling authentication to
//! the long-running daemon. These commands work on the persisted session
//! (`{data_dir}/session.json`) and exit, so a service-managed `jolt headless`
...
use jolt_engine::{AuthState, Engine, EngineConfig, InstanceLock};

/// `jolt login`: authenticate via the paste-code flow, provision the hidden
/// Personal organization, persist `session.json`, and exit.
pub async fn login(config: EngineConfig) -> anyhow::Result<()> {
    ...
    if let AuthState::SignedIn { user, .. } = auth.state() {
        println!("Already signed in as {}.", user.email);
        println!("Run `jolt logout` first to switch accounts.");
        return Ok(());
    }
    if !std::io::stdin().is_terminal() {
        anyhow::bail!("jolt login needs an interactive terminal");
    }
    let _lock = engine_lock(&config, "sign in")?;
    jolt_engine::terminal_sign_in(&auth).await?;
    match auth.state() {
        AuthState::SignedIn { user, .. } => {
            println!("\nSigned in as {}.", user.email);
            println!("Session saved — `jolt headless` (and the daemon) will use it.");
        }
...
/// `jolt logout`: remove the persisted session.
    ...
    InstanceLock::acquire(&config.data_dir).map_err(|err| {
        anyhow::anyhow!(
            "{err}\nCannot {verb} while an engine is running — stop it first \
             (`jolt daemon stop`, or quit the Jolt app), or use the running UI instead."
        )
    })
}
