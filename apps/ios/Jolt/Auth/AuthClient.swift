...
struct AuthClient {
    ...
    func createPersonalOrg(accessToken: String) async throws -> String {
        struct Response: Codable { var organizationId: String }
        var request = URLRequest(url: baseURL.appending(path: "auth/orgs"))
        request.httpMethod = "POST"
        request.setValue("Bearer \(accessToken)", forHTTPHeaderField: "Authorization")
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.httpBody = try JSONEncoder().encode(["name": "Personal"])
        let (data, response) = try await URLSession.shared.data(for: request)
        try Self.check(data: data, response: response)
        return try JSONDecoder().decode(Response.self, from: data).organizationId
    }
...
enum Keychain {
    private static let service = "dev.trmcnvn.jolt.ios"
