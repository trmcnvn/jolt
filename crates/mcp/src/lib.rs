...
pub const SERVER_NAME: &str = "jolt";
pub const GOAL_GET: &str = "goal_get";
pub const GOAL_UPDATE: &str = "goal_update";
pub const GOAL_COMPLETE: &str = "goal_complete";
pub const GOAL_REPORT_BLOCKED: &str = "goal_report_blocked";
pub const GOAL_PAUSE: &str = "goal_pause";
pub const GOAL_RESUME: &str = "goal_resume";
pub const REQUEST_ANSWERS: &str = "request_answers";
...
const MAX_GOAL_STATUS_CHARS: usize = 2_000;
...
pub type McpAnswerRequester = Arc<
    dyn Fn(
...
fn validate_blocker_key(value: &str) -> Result<String, McpError> {
    let key = value.trim();
    if key.is_empty() {
        return Err(McpError::new("blocker key must not be empty"));
    }
...
fn validate_blocker_summary(value: &str) -> Result<String, McpError> {
    let summary = value.trim();
    if summary.is_empty() {
        return Err(McpError::new("blocker summary must not be empty"));
    }
    if summary.chars().count() > MAX_GOAL_STATUS_CHARS {
        return Err(McpError::new(format!(
            "blocker summary exceeds the {MAX_GOAL_STATUS_CHARS} character limit"
        )));
    }
...
impl McpRunTarget {
    fn ensure_active(&self) -> Result<(), McpError> {
        if !self.active.load(Ordering::Acquire) {
            return Err(McpError::new("the MCP run lease has ended"));
        }
        Ok(())
    }

    fn backend(&self) -> Result<&Arc<dyn McpBackend>, McpError> {
        self.ensure_active()?;
        self.backend
            .as_ref()
            .ok_or_else(|| McpError::new("workspace registry is unavailable"))
    }

    fn answer_requester(&self) -> Result<McpAnswerRequester, McpError> {
        self.ensure_active()?;
        self.answer_requester
            .clone()
            .ok_or_else(|| McpError::new("the Jolt answer UI is unavailable"))
    }
...
impl JoltMcpServer {
    ...
    fn target(context: &RequestContext<RoleServer>) -> Result<McpRunTarget, McpError> {
        let parts = context
            .extensions
            .get::<axum::http::request::Parts>()
            .ok_or_else(|| McpError::new("MCP request context is unavailable"))?;
        parts
            .extensions
            .get::<McpRunTarget>()
            .cloned()
            .ok_or_else(|| McpError::new("MCP run context is unavailable"))
    }

    fn parameters<T: DeserializeOwned>(arguments: Option<JsonObject>) -> Result<T, McpError> {
        serde_json::from_value(serde_json::Value::Object(arguments.unwrap_or_default()))
            .map_err(|error| McpError::new(format!("invalid Jolt tool arguments: {error}")))
    }
    ...
    fn failure(error: McpError) -> CallToolResponse {
        CallToolResult::error(vec![ContentBlock::text(error.to_string())]).into()
    }

    fn answer_questions(params: RequestAnswersParams) -> Result<Vec<UserInputQuestion>, McpError> {
        if params.questions.is_empty() {
            return Err(McpError::new(
                "request_answers requires at least one question",
            ));
        }
        ...
        params
            ...
            .map(|question| {
                ...
                if question.multi_select && options.is_empty() {
                    return Err(McpError::new("multiSelect requires at least one option"));
                }
                let mut unique = std::collections::HashSet::new();
                if !options.iter().all(|option| unique.insert(option)) {
                    return Err(McpError::new(
                        "answer options must be unique within a question",
                    ));
                }
    ...
    fn answer_text(value: &str, label: &str, max_chars: usize) -> Result<String, McpError> {
        let value = value.trim();
        if value.is_empty() {
            return Err(McpError::new(format!("{label} must not be empty")));
        }
    ...
    async fn call_answer_tool(
        ...
        let result = async {
            ...
            let answers = requester(questions.clone(), context.ct.clone())
                .await
                .ok_or_else(|| McpError::new("the answer request was cancelled"))?;
        ...
        let result = match request.name.as_ref() {
            ...
            GOAL_RESUME => {
                Self::parameters::<GoalRefParams>(request.arguments).and_then(|params| {
                    target.clear_signal();
                    Self::mutate(&target, params, McpGoalAction::Resume)
                        .map(|goal| Self::success(&goal))
                })
            }
            GOAL_REPORT_BLOCKED => Self::parameters::<GoalBlockedParams>(request.arguments)
                .and_then(|params| {
                    let key = validate_blocker_key(&params.blocker_key)?;
                    let summary = validate_blocker_summary(&params.summary)?;
                    let goal = target
                        .backend()?
                        .goal(&target.chat_id)?
                        .ok_or_else(|| McpError::new("this session has no goal"))?;
                    if goal.id != params.goal_id || goal.revision != params.expected_revision {
                        return Err(McpError::new(
                            "the goal changed before this command applied",
                        ));
                    }
                    if goal.status != GoalStatus::Active {
                        return Err(McpError::new("the goal is not active"));
                    }
...
impl McpLease {
    ...
    pub fn take_goal_signal(&self) -> Option<McpGoalSignal> {
