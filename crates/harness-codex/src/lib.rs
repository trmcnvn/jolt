...
use jolt_proto::{
    AgentCommand, AgentCommandSource, AgentEvent, CommandContext, CostProvenance, DoneStatus,
...
use jolt_harness::environment::HarnessEnvironment;
...
/// PATH in ways a GUI/service launch never sees — see [`jolt_platform::shell_env`]),
...
fn resolve_codex_executable() -> Option<PathBuf> {
    ...
    let mut candidates: Vec<PathBuf> = std::env::var_os("PATH")
        ...
        .unwrap_or_default();
    if let Some(shell_path) = jolt_platform::shell_env::login_shell_path() {
        candidates.extend(
    ...
    candidates.extend(
        jolt_platform::process::node_version_manager_bins()
            .into_iter()
            .map(|d| d.join(exe)),
    );
...
impl Harness for CodexHarness {
    ...
    fn executable_path(&self) -> Result<PathBuf, HarnessError> {
    ...
    async fn commands(&self, context: CommandContext) -> Result<Vec<AgentCommand>, HarnessError> {
        ...
        cmd.arg("app-server");
        jolt_platform::process::compose_child_path(&mut cmd, &exe);
        jolt_harness::environment::apply(&mut cmd, &environment);
    ...
    async fn run(
        ...
        if let Some(mcp) = &controls.mcp {
            cmd.env(jolt_harness::MCP_BEARER_TOKEN_ENV, &mcp.bearer_token);
            cmd.args([
                "-c",
                ...
                &format!(
                    ...
                    mcp.name,
                    jolt_harness::MCP_BEARER_TOKEN_ENV
                ),
            ]);
        }
        ...
        let stdout = child
            ...
            .take()
            .ok_or_else(|| HarnessError::Protocol("codex child has no stdout".into()))?;
        let stderr_tail = jolt_harness::StderrTail::default();
...
fn accumulate_usage(total: &mut Option<AgentEvent>, next: AgentEvent) {
    ...
    match total {
        Some(AgentEvent::Usage {
            ...
            cost_usd: total_cost,
            cost_provenance: total_cost_provenance,
            ...
            *total_cost_provenance = match (*total_cost_provenance, cost_provenance) {
        ...
        if interrupted && !done_after_interrupt {
            ...
            let _ = event_tx
                .send(Ok(AgentEvent::Done {
                    status: DoneStatus::Errored,
                    result: None,
                    error: Some(jolt_harness::crash_message(
                        "codex app-server",
                        status,
                        &stderr_tail,
                    )),
