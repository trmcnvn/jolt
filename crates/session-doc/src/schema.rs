...
//!
//! Part maps: { id, kind: "text"|"textReveal"|"tool"|"input"|"error"|"harnessSwitch"|"changes",
//! text?: LoroText, call?: json, isError?, questions?: json, resolved?, message?, from?, to?, diff? }.
//! Text bodies are **LoroText** so streaming
...
fn to_doc_part(part: &MessagePart) -> Result<DocPartJson, DocError> {
    Ok(match part {
        ...
        MessagePart::TextReveal { id } => DocPartJson {
            id: id.clone(),
            kind: "textReveal".into(),
            ..Default::default()
        },
        MessagePart::Tool {
            id,
        ...
        MessagePart::HarnessSwitch { id, from, to } => DocPartJson {
            id: id.clone(),
            kind: "harnessSwitch".into(),
            from: Some(*from),
            to: Some(*to),
            ..Default::default()
        },
...
fn from_doc_part(p: DocPartJson) -> MessagePart {
    match p.kind.as_str() {
        "textReveal" => MessagePart::TextReveal { id: p.id },
        ...
        "harnessSwitch" => match (p.from, p.to) {
            (Some(from), Some(to)) => MessagePart::HarnessSwitch { id: p.id, from, to },
            _ => MessagePart::Text {
                id: p.id,
                text: String::new(),
            },
...
impl SessionDoc {
    ...
    /// Replace one text part and stamp its message status in a single commit.
    /// Returns `false` when the message or text part does not exist.
    pub fn update_text_message(
        ...
        for i in 0..messages.len() {
            ...
            for j in 0..parts.len() {
                ...
                entry.insert("status", status_str(status))?;
                self.doc.commit();
                return Ok(true);
            }
            return Ok(false);
...
fn push_part(parts: &LoroList, part: &MessagePart) -> Result<(), DocError> {
    ...
    if let Some(from) = doc_part.from {
        map.insert("from", loro_value_from_json(&serde_json::to_value(from)?))?;
    }
    if let Some(to) = doc_part.to {
        map.insert("to", loro_value_from_json(&serde_json::to_value(to)?))?;
    }
...
mod tests {
    ...
    #[test]
    fn text_reveal_part_round_trips() {
        let doc = SessionDoc::init("chat-1").unwrap();
        let mut entry = user_entry("m1", "hello");
        entry.role = MessageRole::Assistant;
        entry
            .parts
            .push(MessagePart::TextReveal { id: "r1".into() });
        entry.status = Some(MessageStatus::Streaming);
        doc.push_message(&entry).unwrap();

        assert_eq!(doc.read_entries().unwrap()[0], entry);
    }

    #[test]
    fn harness_switch_part_round_trips() {
        let doc = SessionDoc::init("chat-1").unwrap();
        let mut entry = user_entry("switch", "");
        entry.role = MessageRole::System;
        entry.parts = vec![MessagePart::HarnessSwitch {
            id: "h0".into(),
    ...
    #[test]
    fn update_text_message_replaces_body_and_completes_entry() {
        let doc = SessionDoc::init("chat-1").unwrap();
        let mut entry = user_entry("m1", "Running…");
        entry.role = MessageRole::System;
        entry.status = Some(MessageStatus::Streaming);
        doc.push_message(&entry).unwrap();

        assert!(
            doc.update_text_message("m1", "t0", "finished", MessageStatus::Complete)
                .unwrap()
        );
        let entries = doc.read_entries().unwrap();
        assert_eq!(entries[0].status, Some(MessageStatus::Complete));
