...
#[derive(Clone)]
pub struct BackgroundServiceController {
    data_dir: PathBuf,
...
impl BackgroundServiceController {
    ...
    pub fn apply_pending(&self) -> anyhow::Result<()> {
        let requested = self
            .pending
            .lock()
...
pub fn relaunch_after_exit() -> anyhow::Result<()> {
    #[cfg(unix)]
    {
        use std::os::unix::process::CommandExt as _;

        let pid = std::process::id().to_string();
        let install = crate::detect_install();
        let mut command = Command::new("/bin/sh");
        match install {
            crate::InstallKind::MacApp { bundle } => {
                command.args([
                    "-c",
            ...
            crate::InstallKind::Managed { app_root } => {
                let executable = app_root.join("current/jolt-desktop");
                command.args([
                    "-c",
