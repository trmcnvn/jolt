...
impl Vcs {
    ...
    pub fn set_selected(&self, kind: VcsKind) -> Result<VcsSettingsSnapshot, VcsError> {
        let commands = discovered_commands();
    ...
    fn save(&self, selected: Option<VcsKind>) -> Result<(), VcsError> {
        ...
        let json = serde_json::to_string_pretty(&StoredSettings { selected })
            .map_err(|err| VcsError::new(format!("VCS settings serialize: {err}")))?;
