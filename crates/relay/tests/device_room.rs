...
use jolt_relay::{
    CLIENT_CLOSED, CLIENT_GONE, DeviceFrameHeader, DeviceLink, HOST_CLOSED, HOST_OFFLINE,
    HostRelay, HostRelayConfig, LinkCache, LinkCacheConfig, NUDGE_KIND, RELAY_KIND, RPC_ZLIB_KIND,
    StaticToken, decode_device_frame, device_room_ws_url, encode_device_frame,
};
...
impl FakeRelay {
    ...
    fn compressed_frames(&self) -> usize {
        self.state.lock().expect("lock").compressed_frames
    }
...
async fn handle_socket(stream: tokio::net::TcpStream, state: Arc<Mutex<RelayState>>) {
    ...
    while let Some(message) = ws_stream.next().await {
        ...
        if header.k == RPC_ZLIB_KIND {
            st.compressed_frames += 1;
        }
...
impl RpcService for TestService {
    async fn handle(&self, method: &str, params: serde_json::Value) -> Result<RpcReply, RpcError> {
        match method {
            ...
            "Large" => Ok(RpcReply::Value(serde_json::json!("x".repeat(64 * 1024)))),
            "Count" => {
...
fn noop_nudge() -> jolt_relay::NudgeHandler {
    Arc::new(|_| {})
}
...
async fn relay_serves_multiple_clients_end_to_end() {
    ...
    let large = a
        .call("Large", serde_json::json!({}))
        .await
        .expect("large compressed response");
    assert_eq!(large.as_str().map(str::len), Some(64 * 1024));
    assert!(relay.compressed_frames() > 0);
...
async fn nudges_reach_the_host_callback() {
    ...
    let on_nudge: jolt_relay::NudgeHandler = Arc::new(move |chat_id| {
...
/// `JOLT_EDGE_WS=http://127.0.0.1:26640 cargo test -p jolt-rpc -- --ignored live_edge`
/// (dev-mode edge; JOLT_EDGE_TOKEN defaults to a fixed dev user id).
#[tokio::test]
#[ignore = "needs a running edge (set JOLT_EDGE_WS)"]
async fn live_edge_relay_round_trip() {
    let Ok(edge_url) = std::env::var("JOLT_EDGE_WS") else {
        panic!("set JOLT_EDGE_WS to the edge base URL (e.g. http://127.0.0.1:26640)");
    };
    let token = std::env::var("JOLT_EDGE_TOKEN").unwrap_or_else(|_| "relay-live-test".into());
