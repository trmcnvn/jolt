...
//! `uleb128(header_len) ‖ UTF-8 JSON header ‖ payload`, header `{s, k, to?, from?, z?}`.
...
use jolt_rpc::{
    MAX_WIRE_FRAME_BYTES, RpcClient, RpcError, RpcService, WireFrame, serve_connection,
};
...
/// Selectively zlib-compressed JSON RPC payload.
pub const RPC_ZLIB_KIND: &str = "rpc-zlib";

const COMPRESSION_THRESHOLD_BYTES: usize = 16 * 1024;
const MAX_HEADER_BYTES: usize = 4 * 1024;
const MAX_HEADER_FIELD_BYTES: usize = 256;
const MAX_DEVICE_FRAME_BYTES: usize = MAX_WIRE_FRAME_BYTES + MAX_HEADER_BYTES + 5;
...
/// (`JSON.stringify` of `{s, k, to?, from?, z?}`); absent routing/capability keys are omitted.
...
pub struct DeviceFrameHeader {
    ...
    /// This peer can receive [`RPC_ZLIB_KIND`] frames.
...
impl DeviceFrameHeader {
    ...
    pub fn with_compression_support(mut self) -> Self {
        self.z = true;
        self
    }
    ...
    fn validate(&self) -> Result<(), RpcError> {
        ...
        if (self.s.is_empty() && self.k != RELAY_KIND)
            || self.k.is_empty()
            || fields
                .into_iter()
                .flatten()
                .any(|field| field.len() > MAX_HEADER_FIELD_BYTES)
        {
            return Err(RpcError::Transport(
                "device frame: invalid header field length".into(),
            ));
        }
        Ok(())
    }
...
pub fn encode_device_frame(header: &DeviceFrameHeader, payload: &[u8]) -> Result<Bytes, RpcError> {
    header.validate()?;
    if payload.len() > MAX_WIRE_FRAME_BYTES {
        return Err(RpcError::Transport(
            "device frame: payload is too large".into(),
        ));
    }
    ...
    if json.len() > MAX_HEADER_BYTES {
        return Err(RpcError::Transport(
            "device frame: header is too large".into(),
        ));
    }
    let mut out = BytesMut::with_capacity(json.len() + payload.len() + 5);
    let mut n = json.len();
    loop {
...
/// Decode a device frame; the payload is a zero-copy slice after the JSON header.
pub fn decode_device_frame(bytes: Bytes) -> Result<(DeviceFrameHeader, Bytes), RpcError> {
    let bad = |m: &str| RpcError::Transport(format!("device frame: {m}"));
    if bytes.len() > MAX_WIRE_FRAME_BYTES + MAX_HEADER_BYTES + 5 {
        return Err(bad("frame is too large"));
    }
    ...
    if len > MAX_HEADER_BYTES {
        return Err(bad("header is too large"));
    }
    ...
    header.validate()?;
    if bytes.len() - header_end > MAX_WIRE_FRAME_BYTES {
        return Err(bad("payload is too large"));
    }
    Ok((header, bytes.slice(header_end..)))
}

fn compress_rpc(payload: &[u8]) -> Result<Bytes, RpcError> {
    let mut encoder = ZlibEncoder::new(Vec::new(), Compression::fast());
    encoder
        .write_all(payload)
        .and_then(|()| encoder.finish())
        .map(Bytes::from)
        .map_err(|error| RpcError::Transport(format!("compress RPC frame: {error}")))
}

fn decompress_rpc(payload: Bytes) -> Result<String, RpcError> {
    ...
    decoder
        ...
        .map_err(|error| RpcError::Transport(format!("decompress RPC frame: {error}")))?;
    if decoded.len() > MAX_WIRE_FRAME_BYTES {
        return Err(RpcError::Transport(
            "decompressed RPC frame is too large".into(),
        ));
    }
    if decoder.total_in() != compressed_len {
        return Err(RpcError::Transport(
            "compressed RPC frame has trailing bytes".into(),
        ));
    }
    String::from_utf8(decoded)
        .map_err(|_| RpcError::Transport("decompressed RPC frame is not UTF-8".into()))
}

fn decode_rpc_text(kind: &str, payload: Bytes) -> Result<String, RpcError> {
    if kind == RPC_ZLIB_KIND {
        decompress_rpc(payload)
    } else {
        String::from_utf8(payload.into())
            .map_err(|_| RpcError::Transport("RPC frame is not UTF-8".into()))
    }
}
...
fn make_virtual_conn(
    ...
    let pump_compression = compression.clone();
    ...
    tokio::spawn(async move {
        while let Some(message) = srv_out_rx.recv().await {
            let (kind, payload) = match message {
                WireFrame::Text(text)
                    if pump_compression.load(Ordering::Acquire)
                        && text.len() >= COMPRESSION_THRESHOLD_BYTES =>
                {
                    match compress_rpc(text.as_bytes()) {
                        Ok(payload) => (RPC_ZLIB_KIND, payload),
                        Err(err) => {
                            tracing::warn!(error = %err, "device-room: RPC compression failed");
                            (RPC_KIND, Bytes::from(text))
                        }
                    }
                }
                WireFrame::Text(text) => (RPC_KIND, Bytes::from(text)),
                WireFrame::Binary(bytes) => (RPC_BINARY_KIND, bytes),
            };
...
async fn handle_host_frame(
    ...
    if header.k != RPC_KIND && header.k != RPC_ZLIB_KIND && header.k != RPC_BINARY_KIND {
    ...
    if header.z || header.k == RPC_ZLIB_KIND {
        conn.compression.store(true, Ordering::Release);
    }
    let message = if header.k == RPC_BINARY_KIND {
        WireFrame::Binary(payload)
    } else {
        match decode_rpc_text(&header.k, payload) {
            Ok(text) => WireFrame::Text(text),
            Err(err) => {
                tracing::warn!(error = %err, "device-room: malformed RPC payload");
                return;
            }
        }
    };
                    ...
                    message = stream.next() => match message {
                        Some(Ok(WsMessage::Binary(bytes))) => {
                            ...
                            match decode_device_frame(bytes) {
                                ...
                                Ok((header, payload))
                                    if header.k == RPC_KIND || header.k == RPC_ZLIB_KIND =>
                                {
                                    let text = match decode_rpc_text(&header.k, payload) {
                                        Ok(text) => text,
                                        Err(err) => {
                                            tracing::warn!(error = %err, "device-room: malformed RPC payload");
                                            continue;
...
impl LinkCache {
    ...
    async fn dial(&self, device_id: &str) -> Result<Arc<DeviceLink>, RpcError> {
        ...
        let probe = client.call(jolt_api::methods::LIST_HARNESSES, serde_json::json!({}));
...
mod tests {
    ...
    #[test]
    fn compresses_large_rpc_payloads_with_a_bounded_decoder() {
        let payload =
            serde_json::json!({ "text": "streaming markdown\n".repeat(2_000) }).to_string();
        let compressed = compress_rpc(payload.as_bytes()).expect("compress");
        assert!(compressed.len() < payload.len() / 4);
        let mut trailing = BytesMut::from(compressed.as_ref());
        trailing.put_u8(0);
        assert_eq!(decompress_rpc(compressed).expect("decompress"), payload);
        assert!(decompress_rpc(trailing.freeze()).is_err());
        assert!(decompress_rpc(Bytes::from_static(b"not zlib")).is_err());
    }
    ...
    fn rejects_malformed_frames() {
        assert!(decode_device_frame(Bytes::new()).is_err()); // empty: truncated uleb128
        assert!(decode_device_frame(Bytes::from_static(&[0x85])).is_err()); // continuation bit, no next byte
        assert!(decode_device_frame(Bytes::from_static(&[10, b'{'])).is_err()); // truncated header
        let mut minimal = vec![15u8];
        minimal.extend_from_slice(br#"{"s":"a","k":"b"}"#[..15].as_ref()); // wrong len: truncated JSON
        assert!(decode_device_frame(Bytes::from(minimal)).is_err());
        let mut valid = vec![17u8];
        valid.extend_from_slice(br#"{"s":"a","k":"b"}"#);
        valid.push(9); // trailing payload byte
        let (h, p) = decode_device_frame(Bytes::from(valid)).expect("valid minimal frame");
        assert_eq!((h.s.as_str(), h.k.as_str()), ("a", "b"));
        assert_eq!(p, Bytes::from_static(&[9]));
        assert!(
            decode_device_frame(Bytes::from_static(&[0xff, 0xff, 0xff, 0xff, 0xff, 0x01])).is_err()
        ); // overflow
        assert!(encode_device_frame(&header(&"x".repeat(257), "rpc"), &[]).is_err());
    }
