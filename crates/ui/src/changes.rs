...
use gpui::{
    AnyElement, App, Context, Entity, EventEmitter, Focusable, ListAlignment, ListScrollEvent,
    ListState, Render, ScrollHandle, SharedString, Subscription, Task, Window, div, font, list,
    prelude::*, px,
};
use jolt_proto::{
    CheckoutDiffManifest, CheckoutDiffPage, CheckoutDiffWatchFrame, DiffCompleteness,
    DiffFileDescriptor, DiffReviewAnchor, DiffReviewComment, DiffReviewExcerptLine,
    DiffReviewLineKind, DiffReviewSnapshot, DiffReviewSubject, InclusiveLineRange,
    REVIEW_DRAFT_SCHEMA_VERSION, ReviewDraft, ReviewDraftTarget, TurnDiffManifest,
};
...
use crate::review::feedback_message;
...
pub const REVIEW_GUTTER_WIDTH: f32 = 24.0;
...
const REVIEW_MESSAGE_MAX_BYTES: usize = 240 * 1024;
...
fn review_key_for_source(source: &DiffSource) -> String {
    match source {
        DiffSource::Checkout { chat_id, .. } => format!("review:diff:working-copy:{chat_id}"),
        DiffSource::Turn {
            chat_id,
            assistant_message_id,
            ..
        } => format!("review:diff:assistant-turn:{chat_id}:{assistant_message_id}"),
    }
}

fn review_subject(source: &DiffSource) -> DiffReviewSubject {
    match source {
        DiffSource::Checkout { chat_id, .. } => DiffReviewSubject::WorkingCopy {
...
fn review_line_kind(kind: LineKind) -> Option<DiffReviewLineKind> {
    match kind {
        LineKind::Context => Some(DiffReviewLineKind::Context),
        LineKind::Add => Some(DiffReviewLineKind::Addition),
        LineKind::Del => Some(DiffReviewLineKind::Deletion),
        LineKind::Meta => None,
    }
}
...
fn excerpt_matches_line(excerpt: &DiffReviewExcerptLine, line: &DiffLine) -> bool {
    review_line_kind(line.kind) == Some(excerpt.kind)
        ...
        && line.new_no == excerpt.new_number
        && line.text == excerpt.text
}

fn review_line_key(file_id: &str, line: &DiffLine) -> Option<ReviewLineKey> {
    Some(ReviewLineKey {
        file_id: file_id.to_string(),
        old_number: line.old_no,
        new_number: line.new_no,
        kind: review_line_kind(line.kind)?,
...
pub struct Changes {
    ...
    newer_manifest: Option<CheckoutDiffManifest>,
    pinning_revision: Option<String>,
    editing_comment: Option<String>,
    comment_editor: Option<Entity<ComposerInput>>,
    comment_editor_subscription: Option<Subscription>,
    review_load_task: Option<Task<()>>,
    review_save_task: Option<Task<()>>,
    review_pin_task: Option<Task<()>>,
    review_sending: bool,
    inflight_reviews: HashMap<String, ReviewDraft>,
    list: ListState,
...
impl Changes {
    pub fn new(state: Entity<AppState>, cx: &mut Context<Self>) -> Self {
        ...
        Self {
            ...
            review: None,
            newer_manifest: None,
            pinning_revision: None,
            editing_comment: None,
            comment_editor: None,
            comment_editor_subscription: None,
            review_load_task: None,
            review_save_task: None,
            review_pin_task: None,
            review_sending: false,
            inflight_reviews: HashMap::new(),
            list,
            _observe: observe,
        }
    }

    fn diff_comments(&self) -> &[DiffReviewComment] {
    ...
    fn diff_comments_mut(&mut self) -> Option<&mut Vec<DiffReviewComment>> {
    ...
    fn set_review_source(&mut self, source: &DiffSource, cx: &mut Context<Self>) {
        ...
        self.review_load_task = Some(cx.spawn(async move |this, cx| {
            let result = engine
                .client()
                .call_as::<Option<ReviewDraft>>(
                    methods::GET_REVIEW_DRAFT,
                    serde_json::json!({ "reviewKey": key }),
                )
                .await;
    ...
    fn apply_loaded_review(&mut self, draft: ReviewDraft, cx: &mut Context<Self>) {
    ...
    fn persist_review_now(&self, cx: &mut Context<Self>) {
    ...
    fn schedule_review_save(&mut self, cx: &mut Context<Self>) {
    ...
    fn line_value(&self, point: &ReviewLinePoint) -> Option<(DiffFileDescriptor, DiffLine)> {
    ...
    fn select_review_line(
        ...
        let mut params = serde_json::json!({
            "chatId": match &source {
                DiffSource::Checkout { chat_id, .. } => chat_id,
                DiffSource::Turn { chat_id, .. } => chat_id,
            },
            "catalogRevision": revision,
            "reviewId": review_id,
        });
        if let DiffSource::Checkout {
            target: Some(target),
            ..
        } = &source
        ...
        self.review_pin_task = Some(cx.spawn_in(window, async move |this, cx| {
            let result = engine
                .client()
                .call(methods::PIN_DIFF_DOCUMENT, params)
                .await;
    ...
    fn update_editing_comment(&mut self, body: String, cx: &mut Context<Self>) {
    ...
    fn edit_comment(&mut self, comment_id: String, window: &mut Window, cx: &mut Context<Self>) {
        self.open_comment_editor(comment_id, window, cx);
        self.rebuild_rows();
        cx.notify();
    }

    fn delete_comment(&mut self, comment_id: &str, cx: &mut Context<Self>) {
        if let Some(comments) = self.diff_comments_mut() {
    ...
    fn delete_persisted_review(&self, draft: ReviewDraft, cx: &mut Context<Self>) {
        let Some(engine) = self.state.read(cx).engine().cloned() else {
            return;
        };
        cx.spawn(async move |_, _| {
            let _ = engine
                ...
                .call(
                    ...
                    serde_json::json!({ "reviewKey": draft.review_key }),
                )
                .await;
            ...
            {
                let mut params = serde_json::json!({
                    "catalogRevision": snapshot.manifest.catalog_revision,
                    "reviewId": draft.review_id,
                });
                if let Some(target) = snapshot.target_device_id
                    && let Some(object) = params.as_object_mut()
                {
                ...
                let _ = engine
                    .client()
                    .call(methods::RELEASE_DIFF_DOCUMENT, params)
                    .await;
            }
        })
        .detach();
    ...
    fn show_newer_manifest(&mut self) {
    ...
    pub fn show_turn_diff(
        ...
        if let Some(source) = self.source.clone() {
            self.set_review_source(&source, cx);
            if let Some(ReviewDraftTarget::Diff { snapshot, .. }) =
                self.review.as_ref().map(|review| &review.target)
            {
                self.manifest = Some(snapshot.manifest.clone());
            }
        }
    ...
    fn sync_watch(&mut self, cx: &mut Context<Self>) {
        ...
        self.manifest = self
            .review
            .as_ref()
            .and_then(|review| match &review.target {
                ReviewDraftTarget::Diff { snapshot, .. } => Some(snapshot.manifest.clone()),
                ReviewDraftTarget::AssistantMessage { .. } => None,
            });
        self.sequence = 0;
        self.expanded.clear();
        self.horizontal_scrolls.clear();
    ...
    fn apply_frame(&mut self, frame: CheckoutDiffWatchFrame) -> Result<(), ()> {
        ...
        let retained_revision = self
            .review
            .as_ref()
            .map(|review| review.snapshot_id.as_str())
            .or(self.pinning_revision.as_deref());
        if retained_revision.is_some_and(|revision| revision != manifest.catalog_revision) {
            self.sequence = sequence;
            self.newer_manifest = Some(manifest);
            self.rebuild_rows();
            return Ok(());
        }
    ...
    fn rebuild_rows(&mut self) {
        ...
        for comment in self.diff_comments() {
            ...
            let key = ReviewLineKey {
                file_id: comment.anchor.file_id.clone(),
                old_number: last.old_number,
                new_number: last.new_number,
                kind: last.kind,
                text: last.text.clone(),
            };
            review_rows.entry(key).or_default().push((
                comment.id.clone(),
...
impl Render for Changes {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let review_count = self.diff_comments().len();
        let has_feedback = self
            .diff_comments()
            .iter()
            .any(|comment| !comment.body.trim().is_empty());
                    ...
                    .when(newer_available, |element| {
                        element.child(
                            div()
                                ...
                                .text_size(px(11.0))
                                .text_color(theme.warning)
                                .child("Newer changes are available · finish or discard this review to update"),
                        )
                    })
                    .child(
                        div()
                                ...
                                .child(
                                    div()
                                        ...
                                        .child(SharedString::from(format!(
                                            "{review_count} pending comment{}",
                                            if review_count == 1 { "" } else { "s" }
                                        ))),
                                )
                                .child(
                                    div()
                                        .id("send-review-feedback")
                                        .px(px(12.0))
                                        .py(px(6.0))
                                        .rounded(px(6.0))
                                        .text_size(px(11.0))
                                        .when(has_feedback && !review_sending, |button| {
                                            button
                                                .cursor_pointer()
                                                .bg(theme.solid)
                                                .text_color(theme.on_solid)
