...
use jolt_api::{
    DeleteReviewDraft, GetCheckoutDiffPage, GetReviewDraft, GetTurnDiffPage, PinDiffDocument,
    PutReviewDraft, ReleaseDiffDocument, WatchCheckoutDiff, call as call_api,
    subscribe as subscribe_api,
};
use jolt_proto::{
...
mod review;
pub use patch::{
    DiffLine, FileDiff, FileStatus, Hunk, LineKind, file_notices, lang_for_path, parse_patch,
};
...
async fn fetch_page(
    ...
    call_api(
        ...
        &GetCheckoutDiffPage {
            ...
            target_device_id: target.map(str::to_owned),
        },
    )
    .await
}
...
impl Changes {
    ...
    fn spawn_watch(
        ...
        cx.spawn(async move |this, cx| {
            loop {
                let request = WatchCheckoutDiff {
                    chat_id: chat_id.clone(),
                    target_device_id: target.clone(),
                };
                match subscribe_api(engine.client(), &request).await {
                    Ok(mut receiver) => {
