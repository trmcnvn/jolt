...
//! handlers don't apply here — jolt renders raw HTML literally and has no
//! math):
...
pub const PENDING_LINK_URL: &str = "jolt:pending-link";
...
mod tests {
    ...
    fn links_mend_to_pending_sentinel() {
        mends("[docs](https://x.dev/lo", "[docs](jolt:pending-link)");
        mends("[docs](", "[docs](jolt:pending-link)");
        mends("see [do", "see [do](jolt:pending-link)");
        mends("![alt](https://x/i.p", "![alt](jolt:pending-link)");
        stays("see [");
    ...
    fn link_urls_allow_nested_parens() {
        ...
        mends("[a](https://x.dev/(y", "[a](jolt:pending-link)");
    }

    #[test]
    fn emphasis_inside_link_text_closes_inside() {
        mends("[**a", "[**a**](jolt:pending-link)");
        mends("**a [b", "**a [b](jolt:pending-link)**");
    }
