...
/// Session-title command center (Cmd+Shift+F): focused query, ranked rows, and
/// keyboard navigation. Results are recomputed from the synced chat index so
...
impl Shell {
    ...
    fn close_spaces_menu(&mut self, cx: &mut Context<Self>) {
        if self.spaces_menu.take().is_some() {
            cx.notify();
        }
    }
    ...
    pub(super) fn render_spaces_filter(
        ...
        let trigger = if self.spaces_menu.is_some() {
    ...
    fn session_search_key(&mut self, event: &gpui::KeyDownEvent, cx: &mut Context<Self>) {
        let key = popover::classify_key(
            event.keystroke.key.as_str(),
            event.keystroke.modifiers.platform,
            event.keystroke.modifiers.control,
        ...
        match key {
            ...
            popover::MenuKey::Enter | popover::MenuKey::ModEnter => {
                self.activate_session_search(cx)
            }
            popover::MenuKey::Backspace | popover::MenuKey::Other => {}
        }
    }

    pub(super) fn render_session_search_overlay(
        &mut self,
        ...
        {
            let flow = self.session_search.as_mut()?;
                    ...
                    div()
                        ...
                        .children(rows.into_iter().enumerate().map(|(index, row)| {
                            ...
                            popover::menu_row_nav(
                                ...
                                false,
                                index == active,
                                format!("session-search-row-{index}"),
                            )
                            .id(("session-search-row", index))
                            .h(px(50.0))
                            .when(index == active, |element| {
                                element.shadow(crate::theme::card_selected_shadows())
                            })
                            .on_click(cx.listener(move |this, _, _, cx| {
                                this.session_search = None;
                                ...
                                div()
                                    ...
                                    .child(
                                        div()
                                            ...
                                            .when_some(
                                                row.harness.map(crate::pickers::harness_brand_icon),
                                                |element, (path, tint)| {
                                                    element.child(
                                                        icon(path)
                                                            .size(px(10.0))
        ...
        let body = div().h(px(330.0)).flex().flex_col().child(list);
        let footer = div()
            .flex_none()
            .bg(band)
            .border_t_1()
        ...
        let card = div()
            ...
            .track_focus(&focus)
            .on_key_down(cx.listener(|this, event: &gpui::KeyDownEvent, _, cx| {
                this.session_search_key(event, cx)
            }))
            .on_mouse_down_out(cx.listener(|this, _, _, cx| {
                this.session_search = None;
                cx.notify();
            ...
            .child(footer)
            .into_any_element();
        Some(popover::modal("session-search-dialog", viewport, card))
    }
