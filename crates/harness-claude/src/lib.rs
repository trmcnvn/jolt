...
/// [`jolt_platform::shell_env`]), then known install locations as a last resort.
...
impl ClaudeHarness {
    ...
    fn build_command(
        ...
        mcp: Option<&jolt_harness::McpServerConfig>,
    ) -> Command {
...
async fn load_image_blocks(paths: &[String]) -> Vec<wire::ImageBlock> {
    ...
    for path in paths {
        ...
        blocks.push(wire::ImageBlock {
            ...
            data: jolt_harness::simd_base64::encode(&bytes),
        });
    }
    blocks
}
        ...
        if interrupted && !done_after_interrupt {
            ...
            let _ = event_tx
                .send(Ok(AgentEvent::Done {
                    ...
                    result: None,
                    error: Some(jolt_harness::crash_message("claude", status, &stderr_tail)),
                    session_id: norm.session_id.clone(),
                }))
                .await;
        }
