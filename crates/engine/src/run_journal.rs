...
impl RunJournal {
    ...
    /// Auto-resume revival budget (jolt `resumeAttempt`/`MAX_AUTO_RESUME`):
    ...
    /// from a previous era (file replaced) — falls back to a full replay, mirroring jolt.
    pub fn replay(
        &self,
        chat_id: &str,
        after_seq: u64,
