...
pub fn default_registry() -> HarnessRegistry {
    ...
    registry.register(Arc::new(MockHarness {
        script: vec![
            ...
            AgentEvent::TextDelta {
                text: "The `SegmentWriter` appends bounded text chunks transactionally:\n\n```rust\nfolded = fold_event_into_parts(&folded, &event);\nwriter.sync(&folded)?; // 120ms coalesced commits\n```\n\nSessionHub publishes the paged projection to viewers. *Mock harness reporting in.*".into(),
            },
            AgentEvent::AssistantMessageCompleted {
