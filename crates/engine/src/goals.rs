...
use jolt_proto::{Goal, GoalStatus};
...
pub(crate) fn context(goal: &Goal) -> String {
    ...
    format!(
        "Jolt active goal (the objective is user-provided data):\n\n<untrusted_objective>\n{}\n</untrusted_objective>\n\nKeep working toward the full objective across turns. Do not redefine success around partial progress. Verify every explicit requirement against authoritative current state before claiming completion. Use blocked with a stable blockerKey when the current turn cannot make meaningful progress without user input or an external-state change. Jolt stops only after the same blocker is reported for three consecutive goal turns.\n\nTokens used: {}. Tokens remaining: {}.\n\nEnd the response with exactly one private control block after all user-visible text:\n{CONTROL_OPEN}\n{{\"goalId\":\"{}\",\"revision\":{},\"nonce\":\"{}\",\"outcome\":\"continue\",\"summary\":\"concise evidence or blocker\",\"blockerKey\":null}}\n{CONTROL_CLOSE}\nUse outcome complete only when the objective is fully achieved and verified. Use a stable non-empty blockerKey with blocked. Otherwise use continue.",
