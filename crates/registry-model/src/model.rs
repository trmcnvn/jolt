...
use jolt_proto::{Chat, ChatConfig, Device, Session, SessionStatus, Space, ThemeFileRecord};

#[derive(Debug, thiserror::Error)]
pub enum RegistryError {
    #[error("registry schema: {0}")]
    Schema(String),
    #[error("registry json: {0}")]
    Json(#[from] serde_json::Error),
}

/// Everything in the workspace registry, materialized.
...
/// Result of deleting a space and its chats.
...
/// Result of deleting a device and its spaces and chats.
#[derive(Debug, Clone, PartialEq)]
...
struct RawChat {
    ...
    #[serde(default)]
    harness_conversations: Vec<jolt_proto::HarnessConversationRef>,
    #[serde(default)]
...
impl From<RawChat> for Chat {
    fn from(raw: RawChat) -> Self {
        Self {
            ...
            harness_conversations: raw.harness_conversations,
...
impl RegistryDoc {
    ...
    pub fn from_bytes(bytes: &[u8], device_id: &str) -> Result<Self, RegistryError> {
    ...
    pub fn read_themes(&self) -> Result<Vec<ThemeFileRecord>, RegistryError> {
    ...
    pub fn upsert_device(&mut self, device: &Device) -> Result<(), RegistryError> {
        let set = fields([
            ("id", json!(device.id)),
    ...
    pub fn rename_device(&mut self, device_id: &str, name: &str) -> Result<bool, RegistryError> {
    ...
    pub fn delete_device(&mut self, device_id: &str) -> Result<DeletedDevice, RegistryError> {
    ...
    pub fn read_devices(&self) -> Result<Vec<Device>, RegistryError> {
    ...
    pub fn upsert_space(&mut self, space: &Space) -> Result<(), RegistryError> {
        let set = fields([
            ("id", json!(space.id)),
    ...
    pub fn space(&self, space_id: &str) -> Result<Option<Space>, RegistryError> {
        Ok(self
    ...
    pub fn read_spaces(&self) -> Result<Vec<Space>, RegistryError> {
    ...
    pub fn delete_space(&mut self, space_id: &str) -> Result<DeletedSpace, RegistryError> {
    ...
    pub fn upsert_chat(&mut self, chat: &Chat) -> Result<(), RegistryError> {
        ...
        let set = fields([
            ...
            (
                ...
                serde_json::to_value(&chat.harness_conversations)?,
            ),
    ...
    /// Claim-on-first-command row containing only fields known by the host.
    /// Omitting unknown fields leaves them unclocked, so an earlier full
    /// `createChat` write can still supply config and title after merging.
    pub fn claim_chat(
        &mut self,
        chat_id: &str,
        cwd: Option<&str>,
    ...
    pub fn chat(&self, chat_id: &str) -> Result<Option<Chat>, RegistryError> {
        Ok(self
    ...
    pub fn rename_chat(&mut self, chat_id: &str, title: &str) -> Result<bool, RegistryError> {
        if !self.row_exists(KIND_CHATS, chat_id) {
            return Ok(false);
        }
        self.write(
    ...
    pub fn set_chat_pinned(&mut self, chat_id: &str, pinned: bool) -> Result<bool, RegistryError> {
        if !self.row_exists(KIND_CHATS, chat_id) {
            return Ok(false);
        }
        self.write(
    ...
    pub fn set_chat_branch(&mut self, chat_id: &str, branch: &str) -> Result<bool, RegistryError> {
        if !self.row_exists(KIND_CHATS, chat_id) {
            return Ok(false);
        }
        self.write(
    ...
    pub fn set_chat_cwd(&mut self, chat_id: &str, cwd: &str) -> Result<bool, RegistryError> {
        if !self.row_exists(KIND_CHATS, chat_id) {
            return Ok(false);
        }
        self.write(
    ...
    /// Replace one harness/cwd continuation while retaining every other
    /// harness conversation attached to the chat.
    pub fn set_chat_harness_conversation(
        &mut self,
        chat_id: &str,
        conversation: &jolt_proto::HarnessConversationRef,
    ) -> Result<bool, RegistryError> {
        let Some(mut chat) = self.chat(chat_id)? else {
            return Ok(false);
        };
        chat.harness_conversations.retain(|existing| {
            existing.harness != conversation.harness
                || existing.device_id != conversation.device_id
                || existing.cwd != conversation.cwd
        });
        chat.harness_conversations.push(conversation.clone());
        self.write(
            KIND_CHATS,
            chat_id,
            OpKind::Update,
    ...
    pub fn delete_chat(&mut self, chat_id: &str) -> Result<bool, RegistryError> {
    ...
    pub fn upsert_session(&mut self, session: &Session) -> Result<(), RegistryError> {
        let set = fields([
    ...
    pub fn read_sessions(&self) -> Result<Vec<Session>, RegistryError> {
    ...
    pub fn read_all(&self) -> Result<WorkspaceState, RegistryError> {
        Ok(WorkspaceState {
