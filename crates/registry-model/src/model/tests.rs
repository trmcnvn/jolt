...
use super::*;
use jolt_proto::{
    Goal, GoalStatus, HarnessConversationRef, HarnessId, SandboxLevel, SessionStatus,
};
...
fn harness_conversations_update_independently() {
    let mut ws = RegistryDoc::new("dev-a");
    ws.upsert_chat(&chat("chat-1", "dev-a")).unwrap();
    let conversation = |id: &str, harness, session: &str| HarnessConversationRef {
        id: id.into(),
        harness,
    ...
    ws.set_chat_harness_conversation(
        "chat-1",
        &conversation("claude-1", HarnessId::ClaudeCode, "claude-session"),
    )
    .unwrap();
    ws.set_chat_harness_conversation(
        "chat-1",
        &conversation("codex-1", HarnessId::Codex, "codex-session"),
    )
    .unwrap();
    let mut updated = conversation("claude-1", HarnessId::ClaudeCode, "claude-session");
    updated.covered_through_message_id = Some("assistant-3".into());
    ws.set_chat_harness_conversation("chat-1", &updated)
        .unwrap();

    let chat = ws.chat("chat-1").unwrap().unwrap();
    assert_eq!(chat.harness_conversations.len(), 2);
    assert_eq!(
        chat.harness_conversations
            .iter()
            .find(|item| item.harness == HarnessId::ClaudeCode)
            .and_then(|item| item.covered_through_message_id.as_deref()),
        Some("assistant-3")
    );
    assert!(
        chat.harness_conversations
            .iter()
            .any(|item| item.harness == HarnessId::Codex)
    );
}
...
#[test]
fn late_create_chat_config_survives_prior_claim() {
