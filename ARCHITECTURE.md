...
A Jolt fork of Jolt's ground-up native rewrite — a multi-device controller for coding agents
(Claude Code / Codex / Pi) — in Rust, with a gpui UI. Fresh app; no backwards compatibility required.
...
- Feature parity with jolt **except token-usage display** (poor fit for CRDTs; excluded).
...
- **Engine = backend** (was `@jolt/backend`): runs agents, owns auth, terminals, repos/worktrees,
...
- **Edge (TypeScript, ported from jolt `apps/edge`)**: Worker + SessionRoom DO (per chat) +
...
### Headed / headless
Single binary `jolt`:
- `jolt` — headed. If a local engine daemon is already listening on the IPC port, connect to it;
...
- `jolt headless` — engine only; prints sign-in URL on TTY (paste-code flow), serves IPC on
...
2. **Workspace doc** (per org — NEW; replaces jolt's residual entity sync) — **spaces**
...
3. **Mirror layer** (`jolt-doc` crate) — Rust equivalent of loro-mirror: typed structs for the
   schema, **incremental** application of `doc.subscribe` diffs into cached state (no full
   re-hydration per change — this is also what fixes jolt's known O(transcript) re-projection
...
This is jolt's proven design, kept verbatim.
...
jolt/
  ...
  crates/
    proto/        jolt-proto    # wire types: AgentEvent, ToolCall, RunRequest, Model,
    ...
    doc/          jolt-doc      # session-doc + workspace-doc schemas, mirror layer,
                                 # parts fold, continuations, command ledger, sidecars
    sync/         jolt-sync     # loro room client (join/VV backfill/fragments/backoff),
                                 # ephemeral presence, DocsStore (SQLite snapshots +
                                 # processed-command ledger)
    harness/      jolt-harness  # Harness trait + claude-code (stream-json subprocess),
                                 # codex (app-server JSON-RPC), Pi (RPC JSONL), mock; steering mailbox,
                                 # requestInput, models/reasoning/options catalogs
    engine/       jolt-engine   # sessions engine (pub/sub, run journal, recovery, stall
    ...
    rpc/          jolt-rpc      # UiRpc/ControlRpc: typed req/resp/stream over WS (tokio-
                                 # tungstenite) + in-memory transport; device-room virtual
                                 # sockets ({s,k,to,from} frames)
    ui/           jolt-ui       # gpui app: shell, sidebar, conversation, composer,
                                 # terminal view, diff pane, settings, animation kit
  apps/
    jolt/                       # the `jolt` binary (headed default, `headless` subcommand)
  edge/                          # TypeScript Worker + DOs (ported from jolt/apps/edge,
...
- **Markdown** (`jolt-ui::markdown`): `pulldown-cmark` parsing on `background_spawn` with
...
- **Terminal**: `libghostty-vt` (Ghostty VT state machine, MIT) + `portable-pty` on the
...
- **Animation kit** (`jolt-ui::motion`): small helpers over gpui `Animation` reproducing the
  jolt catalog — `fade-in` (0.5s, cubic-bezier(0.16,1,0.3,1), translateY 4→0), `splash-out`,
  `jolt-pulse` staggered cell wave (boot splash + loaders), `gradient-spin-pulse` matrix
...
Direct ports of jolt behaviors (spec: feature-inventory §3):
...
- **Doc host**: per-chat handle (join room, VV backfill, write user entries + stream assistant
  ...
  snapshot store.
- **Harness** (`docs/research/harness.md`): trait mirroring jolt's
  `HarnessShape`; Claude Code via `claude` CLI stream-json in/out (control protocol for
  permissions/AskUserQuestion→requestInput, resume, steering); Codex via app-server JSON-RPC;
  Pi via `pi --mode rpc` with dynamic provider-qualified models, project trust, extension UI,
  resume, steering, and abort.
- **Repos/diffs**: runtime-selectable `jj`/`git` subprocess backends (one per device,
  JJ preferred); Git worktrees under `~/.jolt/worktrees`, JJ workspaces under
  `~/.jolt/workspaces`; fs watchers (`notify`) + active-pane polling; diff
...
- **Auth**: WorkOS through edge routes (`/auth/exchange`, `/auth/refresh`, orgs); loopback
  callback server headed, paste-code headless; every user has one hidden organization (the sole
  membership is adopted, or `Personal` is created automatically); dev mode uses the configured
  user id as its bearer.

## 6. Edge plan (TypeScript, `edge/`)

Port `jolt/apps/edge` nearly verbatim (it is already Loro-native and smoke-tested: session room
...
- **Changed**: Postgres entity sync/server → workspace doc + edge; Electron/React/mugen → gpui with
  ported techniques; Node harness SDKs → subprocess protocols; WebRTC → device-room relay (jolt
...
- ✅ **M1 Doc + sync core** — `jolt-doc` mirror over loro 1.13; room client syncs with the edge
...
- ✅ **M2 Engine core** — Claude harness end-to-end headless: `jolt headless` + dev auth runs a
...
- ✅ **M4 Multi-device** — device-room host/client virtual sockets, remote device control, workspace
  doc entity sync, WorkOS auth + automatic hidden-organization setup, presence. Proven live by `scripts/e2e-smoke.sh`:
...
- 🟡 **M5 Full surface** — terminals, diff pane, repo/branch/folder pickers + worktrees,
  agent accounts UI, settings (devices/shortcuts/archived), Codex and Pi harnesses. Gaps: composer
...
3. Cursor harness (jolt has it; CLI surface for Rust TBD) — parity item, scheduled after Codex.
